...
(defn create-person [data options root]
  (let [id (-> data :institutional_id str)
    ...
    (logging/info "Creating person " id " with data " (cheshire/generate-string data))
    (-> root
        (roa/relation :people)
        (roa/request {} :post
    ...
    (doseq [id (set/intersection (-> ipeople keys set)
      ...
      (let [lperson (get lpeople id)
        ...
        (when (not= lperson-params iperson-params)
          (logging/info "Updating person " (str id) " " (cheshire/generate-string iperson-params)
...
(defn delete-people
  ...
  (let [ipeople (get-institutional-people root)]
    (doseq [id (set/difference (-> ipeople keys set)
                               (-> lpeople keys set))]
      (logging/info "Deleting person " (cheshire/generate-string (get ipeople id)))
